...
SIZES = tuple(settings.THUMBNAIL_SIZES)
...
class SceneThumbnailTests(TestCase):
    def setUp(self):
        ...
        self.addCleanup(self._cleanup_tmp_files)

    def _cleanup_tmp_files(self):
        for path in self._tmp_files:
            path.unlink(missing_ok=True)

    def _img_tmp(self, width=640, height=360, suffix=".png") -> Path:
    ...
    def _bytes_tmp(self, data=b"not an image", suffix=".mp4") -> Path:
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as f:
            f.write(data)
            f.flush()
            path = Path(f.name)
    ...
    def test_thumbnails_created_on_image_upload(self):
        storage_provider.upload(self.scene.media_path, self._img_tmp())
        ...
        for key in SIZES:
            ...
            with self.storage.open(thumb) as fh:
                img = Image.open(fh)
                img.load()
            self.assertLessEqual(img.width, settings.THUMBNAIL_SIZES[key][0])
    ...
    def test_non_image_has_no_thumbnails(self):
        storage_provider.upload(self.scene.media_path, self._bytes_tmp(suffix=".mp4"))
        name = self.scene.media_path.name
    ...
    def test_serializer_includes_thumbnails(self):
        storage_provider.upload(self.scene.media_path, self._img_tmp(), save=True)
